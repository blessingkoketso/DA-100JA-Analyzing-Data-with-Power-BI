...
5. テーブルのビジュアルを作成するには、「**フィールド**」 ペインの 「**Product**」 テーブル内から、「**Category**」 フィールドをチェックします。

	![画像 329](Linked_image_Files/PowerBI_Lab04A_image4.png)

	*これ以降、ラボではフィールドの参照に省略表記を使用します。次のようになります。**Product | Category***。

6. テーブルに列を追加するには、「**フィールド**」 ペインで、**Sales | Sales** をチェックします。
...
14. 「**カーディナリティ**」 ドロップダウン リストで、「**一対多**」 が選択されていることに注意してください。
...
1. モデル ビューの 「**フィールド**」 ウィンドウで、必要に応じて 「**Product**」 テーブルを展開します。   

2. 階層を作成するには、「**フィールド**」 ウィンドウで 「**Category**」 列を右クリックし、「**階層の作成**」 を選択 します。
...
4. 階層に 2 番目のレベルを追加するには、「**階層**」 ドロップダウン リストで 「**Subcategory**」 を選択します。   
...
7. 「**フィールド**」 ペインで、「**Products**」 階層に注目してください。
...
9. 列を表示フォルダーに編成するには、**フィールド** ペインで、まず**Background Color Format**列を選択します。

10. **Ctrl** キーを押しながら、「**Font Color Format**」 を選択します。
...
2. 階層の中の**Country**レベルではなく、**Country**列を選択します。
...
2. **geography** という名前の 2 つ目の階層を作成します。次の 4 つのレベルがあります。
...
4. **プロパティ** ペインの **書式設定** セクション内から、**桁区切り記号** プロパティを **はい** にスライドします。

	![画像 357](Linked_image_Files/PowerBI_Lab04A_image28.png)

5. 「**UnitPrice**」 列を選択します。 

6. 「**プロパティ**」 ペインの 「**書式設定**」 セクション内で、「**小数点以下表示桁数**」 プロパティを **2** にスライドします。
...
1. 「**Ctrl**」 キーを押しながら、次の 13 列 (複数のテーブルにまたがる) を選択します。
	...
	* Salesperson | EmploeyeeKey

	* Salesperson | UPN

	* SalespersonRegion | EmploeyeeKey

	* SalespersonRegion | SalesTerritoryKey

	* ターゲット | EmployeeID

2. 「**プロパティ**」 ペインで、「**非表示**」 プロパティを 「**はい**」 にスライドします。
...
4. 「**Properties**」 ペインの 「**書式設定**」 セクション内から、「**小数点以下表示桁数**」 プロパティを **0** (ゼロ) に設定します。
...
2. 「**フィールド**」 ペインで、次のことに注意してください。
	...
	* **Region** および **Reseller** テーブルの空間フィールドは、空間アイコンで装飾されています

	* シグマ記号 (Ʃ)で飾られたフィールドは、既定で要約されます

	* カーソルを **Sales**| **Cost** の上に置くと、ツールチップが表示されます フィールド
...
1. 「**フィールド**」 ペインで、「**Sales**」 テーブルを右クリックし、「**新しいクイック メジャー**」 を選択します。
...
10. 「**Sales**」 テーブルで、次の要件に基づいて 2 番目のクイック メジャーを追加します。 
	...
	* **numerator** を  「**Sales | Profit**」 フィールドに設定する 

	* **denominator**を**Sales | Sales**フィールドに設定します

	* メジャーの名前を **Profit Margin**として変更する

	![画像 372](Linked_image_Files/PowerBI_Lab04A_image44.png)

	![画像 373](Linked_image_Files/PowerBI_Lab04A_image45.png)

11. **Profit Margin** メジャーが選択されていることを確認してから、**メジャーツール** コンテキスト リボンで、書式を小数点以下 **2** 桁の **パーセンテージ** に設定します。  
...
14. 右のガイドをクリックしてドラッグすると、テーブルのビジュアルが広がります。
