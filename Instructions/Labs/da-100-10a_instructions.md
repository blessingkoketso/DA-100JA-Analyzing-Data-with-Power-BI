...
25. 「**タイルの詳細**」 ウィンドウ (右側) の 「**サブタイトル**」 ボックスに「**FY2020**」と入力します。     

	![画像 19](Linked_image_Files/PowerBI_Lab10A_image17.png)

26. ペインの下部で、「**適用**」 をクリックします。

	![画像 20](Linked_image_Files/PowerBI_Lab10A_image18.png)

27. **売上 YTD** タイルにサブタイトルが表示されることに注意してください。

	![画像 21](Linked_image_Files/PowerBI_Lab10A_image19.png)

28. **売上、利益率**タイルの、タイルの詳細を編集します。 

29. 「**タイルの詳細**」 ウィンドウの 「**機能**」 セクションで、「**前回の更新時刻を表示する**」 をオンにします。
...
31. タイルは、最後の更新時間 (Power BI Desktop でデータ モデルを更新するときに行った) を示しています。
...
38. **D:\DA100\Setup** フォルダー内で、 **UpdateDatabase-3-AddPartnerAccount.ps1** ファイルを右クリックし、「**PowerShell で実行**」 を選択します。

39. ダイアログ ボックスが表示されたら、教室のパートナーのアカウント名を入力し、**Enter** キーを押します。 
...
41. 「**ホーム**」 リボンの 「**クエリ**」 グループから、「**更新**」 をクリックします。

	![画像 30](Linked_image_Files/PowerBI_Lab10A_image27.png)

42. 更新が完了したら、Power BI Desktop ファイルを保存します。

43. ファイルを 「**販売分析**」 ワークスペースに発行します。

44. データセットを置き換えるメッセージが表示されたら、「**置換**」 をクリックします。 

	![画像 31](Linked_image_Files/PowerBI_Lab10A_image28.png)

	*Power BI サービスのデータセットには、2020 年 6 月の売上データが含まれるようになりました*。

45. Power BI Desktop を閉じます。

46. Microsoft Edge の Power BI サービスの 「**売上分析**」 ワークスペースで、**売上分析**レポートも公開されていることがわかります。

	このレポートは、**ラボ 05A** および **ラボ 06A** で開発したモデルをテストするために使用したものです。

47. **売上分析**レポート (データセットではありません) を削除します。
...
49. 「**売上、利益幅**」 タイルのサブタイトルで、データが  「**今**」 更新されたことを確認します。

50. また 「**2020 年 6 月**」 の列が表示されていることにも注意してください。

	![画像 33](Linked_image_Files/PowerBI_Lab10A_image29.png)

	*「**売上 YTD**」 タイルのアラートもトリガーされているはずです。しばらくすると、設定したしきい値を売上が超えたことをアラートで通知されます*。

51. **売上 YTD** タイルが **$37M** に更新されたことを確認します。

52. 「**売上 YTD**」 タイルにアラート通知アイコンが表示されていることを確認します。
