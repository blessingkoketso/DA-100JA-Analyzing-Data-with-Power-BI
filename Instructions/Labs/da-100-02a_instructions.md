...
13. 「**SQL Server Database**」 ウィンドウの 「**サーバー**」 ボックスに、「**localhost**」と入力します。
...
15. 既定の認証は、「**現在の資格情報を使用**」 であることに注意してください。
...
17. 暗号化のサポートに関するメッセージが表示されたら、「**OK**」 をクリックします。

	![画像 27](Linked_image_Files/PowerBI_Lab02A_image14.png)

18. 「**ナビゲータ**」 ウィンドウの左側で、**AdventureWorksDW2020** データベースを展開します。
...
19. 「**DimEmployee**」 テーブルを選択します (ただし、チェックはしません)。

	![画像 29](Linked_image_Files/PowerBI_Lab02A_image16.png)

20. 右側のペインで、テーブルのプレビューに注目します。

	*プレビューでは、列と行のサンプルを決定できます*。

21. クエリを作成するには、次の 6 つのテーブルを確認します。
...
22. 選択したテーブルのデータに変換を適用するには、「**データの変換**」 をクリックします。
...
33. 「**クエリ**」 ウィンドウで、「**DimEmployeeSalesTerritory**」 クエリを選択します。
...
38. 列の値を表示するには、「**表示**」 リボン タブの 「**データ プレビュー**」 グループ内から、「**列プロファイル**」 をオンにします。

	![画像 41](Linked_image_Files/PowerBI_Lab02A_image28.png)

39. 「**業務の種類**」 列ヘッダーを選択します。

40. データ プレビュー ウィンドウの下に新しいウィンドウが開きます。

41. 列の統計と値の分布を確認します。

42. データ品質の問題に注意してください: warehouse に対して 2 つのラベルがあります (**Warehouse**、またスペルミスがあります **Ware House**)。

	![画像 51](Linked_image_Files/PowerBI_Lab02A_image29.png)

43. カーソルを 「**Ware House**」 バーの上に置くと、この値を持つ 5 つの行があることに注意してください。

	*次のラボでは、変換を適用して、これらの 5 つの行のラベルを変更します*。

44. 「**クエリ**」 ウィンドウで、「**DimSalesTerritory**」 を選択します。
...
45. 「**クエリ**」 ウィンドウで、「**FactResellerSales**」 クエリを選択します。
...
46. **TotalProductCost** 列の列の品質を確認し、行の 8% が空であることを確認します。 
...
48. 「**開く**」 ウィンドウで、**D:\DA100\Data** フォルダーに移動し、 **ResellerSalesTargets.csv** ファイルを選択します。

49. 「**開く**」 をクリックします。

50. 「**ResellerSalesTargets.csv**」 ウィンドウで、データ プレビューに注目してください。 

51. 「**OK**」 をクリックします。

	![画像 71](Linked_image_Files/PowerBI_Lab02A_image34.png)

52. 「**クエリ**」 ウィンドウで、**ResellerSalesTargets** クエリが追加されていることに注目してください。   
...
53. 空の値を含む列がない点に注意してください。

	*月間販売目標がない場合は、代わりにハイフン文字が格納されます*。

54. 列名の左側にある各列ヘッダーのアイコンを確認します。
