...
15. **Region | Region**フィールドに基づいて、2 番目のスライサーを作成する (階層の**地域** レベルではありません)。  
...
26. 「**フィールド**」 ウィンドウから、「**Sales**」 をドラッグする **|** 「**Profit Margin**」 フィールドを 「**行の値**」 ウェルに入力します。  
...
28. ビジュアル フィールド ペインの 「**共有軸**」 ウェルで、「**Month**」 フィールドの下向き矢印をクリックし、「**データのないアイテムを表示**」 を選択します。     
...
3. **Region | Region**フィールドに基づいてスライサーを追加する。
...
7. **Date | Fascal**をマトリックス **行**ウェルに追加する。
...
10. **フィールド** ウィンドウから、「このページをフィルター」** ウェルに**「Product | Category」**フィールド をドラッグする。
...
2. レポートのデザインとテスト中に行レベルのセキュリティ フィルターをシミュレートするには、「**このページでフィルター**」 ウェル内の 「**フィルター**」 ウィンドウに**Salesperson (Performance) | Salesperson** フィールドを追加します。    
...
13. 視覚化の種類を変更するには、**「視覚化」** ウィンドウで、**「集合縦棒グラフ」** を選択します。
...
4. レポートを **「Sales Analysis」** ワークスペースに公開します。
...
2. **Region** スライサーで、**「Ctrl」** キーを押しながら複数のリージョンを選択します。
...
15. **「Region」** スライサーは、「概要」 ページの **「Region」** スライサーとは異なる選択を行うことに注意してください。
