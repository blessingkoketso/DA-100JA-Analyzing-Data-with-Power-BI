...
4. マトリックスのビジュアル フィールドを構成するには、「**フィールド**」 ウィンドウから 「**Region | Regions**」 をビジュアル内にドラッグ＆ドロップします。

5. 「**Sales | Sales**」 フィールドも追加。

6. 階層全体を展開するには、マトリックス ビジュアルの右上にあるフォークされた二重矢印アイコンを 2 回クリックします。

	![画像 47](Linked_image_Files/PowerBI_Lab06B_image3.png)

* **Regions**階層には、 **Group**、**Country**、および **Region** のレベルが含まれることを思い出してください*。
...
1. 次の式に基づいて、**販売**テーブルにメジャーを追加します。
	...
	*この数式では、メジャーは 、変更されたフィルター コンテキストの 「**Sales**」 列の合計を評価し、「**Region**」 テーブルに適用されているフィルターをすべて削除します。*
...
4. 「**フィールド**」 ウィンドウで、「**Sales All Region**」 メジャーが選択されていることを確認し、数式バーでメジャー名と数式を次の数式に置き換えます。   
	...
	**DAX**
	...
	```   
	*メジャーの名前が変更され、更新された数式が正確に反映されます。**DIVIDE()** 関数は 、**Sales** メジャー (フィルタ コンテキストによって変更されていない) を 「**Region**」 テーブルに 適用されたフィルターを削除する変更されたコンテキストで **Sales** メジャーで除算します*。
...
9. 「**Sales % Country**」 メジャーの計算式は、「**Sales % All Region**」 メジャーの計算式とは若干異なっていることに注意してください。 
	...
	*違いは、分母が**Region**テーブルのすべての列ではなく、**Region**テーブルの 「**Region**」 列のフィルターを削除することによって、フィルター コンテキストを変更することです。これは、グループまたは国の列に適用されるフィルターが保持されることを意味します。これは、国の割合として売上を表す結果を達成します*。
...
16. このメジャーの視覚的な可読性を向上させるには、この改良された式で **Sales ％ Group** メジャーを上書きします。
...
4. 年内の売上値の蓄積に注目してください。
	...
	*TOTALYTD() 関数は、フィルター操作、特に時間フィルター操作を実行します。たとえば、2017 年 9 月 (会計年度の 3 番目の月) の  YTD の売上を計算するには、「**Date**」 テーブルのすべてのフィルター が削除され、会計年度の最初 (2017 年 7 月 1 日) から開始する日付の新しいフィルターに置き換えられ、コンテキスト内の日付期間 (2017 年 9 月 30 日) まで延長されます*。
...
1. 次の式に基づいて、 **Sales** テーブルにメジャーを追加します。
	...
	**DAX**
	...
	RETURN
        SalesPriorYear
	```
	***Sales YoY Growth** メジャーの式は、変数を宣言します。変数は数式ロジックを簡略化するのに役立ち、数式内で式を複数回評価する必要がある場合により効率的です (YoY Growthロジックの場合に該当) 。変数は一意の名前で宣言され、メジャー式は **RETURN** キーワードの後に出力する必要があります*。

	***SalesPriorYear** 変数には、フィルター コンテキストのそれぞれの日付から 12 か月前にシフトするために PARALLELPERIOD() 関数を使用する、修正されたコンテキストでの 「**Sales**」 列の合計を計算する式が割り当てられます*。

2. **Sales YoY Growth** メジャーをマトリックス ビジュアルに追加します。

3. 新しいメジャーは、最初の 12 か月間は空白を返します (会計年度の 2017 年以前には販売が記録されていません)。

4. **2018 年 7 月** の **Sales YoY Growth** メジャー値 が **2017 年 7 月** の **売上**値であることに注意してください。
...
3. 「**Power BI に発行**」 ウィンドウで、「**Sales Analysis**」 ワークスペースを選択します。
...
7. Microsoft Edge の Power BI サービスの 「**ナビゲーション**」 ペイン (左側にある) で、「**Sales Analysis**」 ワークスペースの内容を確認 します。
...
8. **Sales Analysis**レポートの上にカーソルを置き 、垂直省略記号 (..) をクリックして、「**削除**」 を選択します。

	![画像 4](Linked_image_Files/PowerBI_Lab06B_image21.png)

9. 削除の確認を求められたら、「**削除**」 をクリックします。
